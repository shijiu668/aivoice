...
export default function IceSpiceVoiceGenerator() {
  ...
  const handleGenerate = async () => {
    if (!text.trim()) {
      setError('Please enter some text for Ice Spice AI voice generation');
      return;
    }
    ...
    try {
      const requestBody = {
        text: text.trim(),
        celebrityId: 8, // Ice Spice's ID (需要在celebrities.js中添加)
        language: 'en',
        cleanup_voice: true
      };
      ...
      if (!response.ok) {
        let errorMessage = 'Failed to generate Ice Spice AI voice';
        try {
      ...
      setError('Failed to generate Ice Spice AI voice. Please try again. Error: ' + err.message);
    } finally {
  ...
  const handleDownload = () => {
    if (generatedAudio) {
      ...
      link.download = 'ice-spice-ai-voice.wav';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 items-center">
          ...
          <div className="lg:pr-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-6">
              Generate Ice Spice AI Voice
            </h2>
            
            <div className="space-y-6">
              <div>
                <label className="block text-lg font-medium text-gray-900 mb-3">
                  Enter text for Ice Spice voice generator:
                </label>
                <textarea
                  value={text}
                  onChange={(e) => setText(e.target.value)}
                  placeholder="Enter the text you want Ice Spice AI voice to speak..."
              ...
              >
                {isGenerating ? (
                  <>
                    ...
                    <span>Generating Ice Spice Voice...</span>
                  </>
                ) : (
                  <span>Generate Ice Spice Voice</span>
                )}
              </button>
            </div>
          </div>

          {/* Right Side - Ice Spice Photo */}
          <div className="lg:pl-12 mt-8 lg:mt-0">
            <div className="text-center">
              <img 
                src="/avatars/ice-spice.jpg" 
                alt="Ice Spice AI Voice Generator"
                className="w-80 h-80 rounded-2xl object-cover shadow-2xl mx-auto"
              />
              <h3 className="text-2xl font-bold text-gray-900 mt-6">Ice Spice</h3>
              <p className="text-lg text-gray-600 mt-2">Rapper & Artist</p>
      ...
      {generatedAudio && (
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-white/50">
          <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">
            Your Ice Spice AI Voice is Ready!
          </h3>
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="flex space-x-4">
              ...
              >
                ...
                <span>Download Ice Spice Voice</span>
              </button>
              <button
                onClick={() => {
